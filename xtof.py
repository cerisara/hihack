    ...
    with tqdm(total=total_rollouts, position=idx, desc=str(os.getpid())) as pbar:
        for game_id in range(start_i, start_i + total_rollouts):
            ...
            if game_id >= seeds.shape[0]:
                print("bug",game_id,seeds.shape[0])
                break
...
def parse_args():
    ...
    parser.add_argument('-c', '--cores', default=1, type=int, help='cores to employ')
...
def main():
    args = parse_args()
    if False: create_dataset(args)
    else:
        if args.role is None: role = 'null'
        else: role = args.role
        relevant_seeds = get_seeds(args.episodes, role, args.seed)
        zbase=int(np.log10(args.episodes) + 0.5)
        gen_and_write_episode(0, 0, relevant_seeds.shape[0], "data", relevant_seeds, zbase)
